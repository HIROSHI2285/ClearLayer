...
- **プロレベルの自動背景削除**: BiRefNet (Lite) と Guided Filter (Alpha Matting) を組み合わせることで、髪の毛のような細かいディテールも1クリックで正確に切り抜きます。
- **AIスマート選択 (SAM 2.1)**: 最新の Segment Anything Model 2.1 を搭載。画像をクリックするだけで、抽出したい範囲を直感的に指定できます。
- **クリエイティブ・ツール**:
  - **切り抜きツール**: 必要な範囲を矩形選択して抽出。
  - **手動消しゴム**: ブラシを使って透明度を微調整。
